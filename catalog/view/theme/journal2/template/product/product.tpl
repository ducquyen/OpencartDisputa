          ...
          <?php } ?>
            <div class="form-group cart <?php echo isset($labels) && is_array($labels) && isset($labels['outofstock']) ? 'outofstock' : ''; ?>">
              ...
              <script>
                ...
                <button type="button" id="button-cart" style="width:150px;float:left" data-loading-text="<?php echo $text_loading; ?>" class="button col-md-2"><span class="button-cart-text"><?php echo $button_cart; ?></span></button>
		<button type="button" id="disputa-button" style="width:70px;height:40px" class="button" data-toggle="tooltip" data-placement="right" title="Cobertura de Preço"><img src="http://i.imgur.com/JsAppos.png?1"></button>
                <?php endif; ?>
              </div>
            </div>
            <?php if ($minimum > 1) { ?>
            ...
            <?php } ?>
	    <div id="disputa-panel" style="display:none">
		<h3>Encontrou oferta melhor? Nós cubrimos.</h3>
		Informe a url que encontrou e o valor e vamos fazer o possível e o impossível para cubrir o preço
		e entraremos em contato com você.<br>
		URL do melhor preço :
		<input type="text" id="urlDisputa" placeholder="http://www.xxx.com.br?produto=y" class="form-control">
		E-mail de contato :
		<input type="text" id="mailDisputa" placeholder="xxx@y.com" class="form-control">
		<input type="hidden" id="product_name" value="<?php echo $heading_title; ?>" />
		<button type="button" id="salvarDisputa" class="button">Solicitar Cobertura</button>
		<span id="resultado"></span>
	    </div>
<script type="text/javascript">
	/* Scripts do módulo de disputa */
	$(document).ready(function(){
		//clear inputs
		$('#urlDisputa').click(function(){
			$(this).attr('value','');
		});

		$('#mailDisputa').click(function(){
		       $(this).attr('value','');
		});
		
		//panel visibility
		$('#disputa-button').click(function(){
			$('#disputa-panel').slideDown();		
		});

		//save data
		$('#salvarDisputa').click(function(){
			//validate form
			errors = 0;
			if(($('#urlDisputa').val() == '')) {
				$('#urlDisputa').css('border','solid red 1px');
				errors++;
			} else {
				$('#urlDisputa').css('border','0px');
			}
			if(($('#mailDisputa').val() == '')){
				$('#mailDisputa').css('border','solid red 1px');
				errors++;
			} else {
				$('#mailDisputa').css('border','0px');
			}
